...
def formatEmbDict(filepath):
  ...
  for string in content:
    word = string.split()
    wordDict[word[0]] = np.array(word[1:], float)
...
def myModel(vocab_size, embedding_matrix, maxlen):
  ...
  model.add(Flatten())
  model.add(Dense(units=128, activation='sigmoid', kernel_regularizer=regularizers.l2(0.002))) 
  model.add(Dense(units=128, activation='sigmoid', kernel_regularizer=regularizers.l2(0.002)))
  model.add(Dense(units=1, activation='sigmoid', kernel_regularizer=regularizers.l2(0.002)))
